...
P8.  -- 2nd value per group: NTH_VALUE(weight, 2) OVER (PARTITION BY breed ORDER BY weight ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)
...
P32. SELECT Customer_Name, SUBSTRING_INDEX(Customer_Name, ' ', 1) AS first_name, SUBSTRING(Customer_Name, LOCATE(' ', Customer_Name) + 1) AS last_name FROM orders; -- SUBSTRING_INDEX(str, delim, 1) takes everything before the first space. LOCATE finds the position of the space. SUBSTRING takes from position+1 to end. --  'extract first name and last name from full name', 'split on space'



P33.ROUND(SUM(CASE WHEN source='bsearch' THEN 1 ELSE 0 END) /. -- 'bsearch as % of gsearch', 'ratio that could be 0 in denominator'
...
P34. Always LOWER() before LIKE to avoid case issues. DATE_FORMAT('%D %M %Y') gives '1st January 1999' format.

...
A Candidate Key is any column or combination of columns that can uniquely identify every row in a table. A table can have multiple candidate keys. Each candidate key qualifies to become the primary key.
A Primary Key is the one candidate key that the designer selects as the official row identifier. A table can have only one primary key. The primary key column does not allow NULL values and does not allow duplicate values.
A table can have only one primary key but multiple candidate keys. The primary key cannot contain NULL values, whereas a candidate key that is not chosen as the primary key may allow NULLs depending on the design. All primary keys are candidate keys, but not all candidate keys are primary keys. The primary key is automatically enforced by the RDBMS through a unique index, and it is commonly used as the target of foreign key references from other tables.

What is the difference between primary key and unique key?
A PRIMARY KEY uniquely identifies each row in a table. It automatically enforces both the NOT NULL and UNIQUE constraints. A table can have exactly one primary key. The primary key is the main reference point for foreign keys in other tables. The column designated as the primary key cannot contain a NULL value under any circumstances.
A UNIQUE KEY (or UNIQUE constraint) also ensures that no two rows have the same value in the constrained column or combination of columns. However, a UNIQUE key column can contain NULL values — typically one NULL is permitted because NULL is not considered equal to another NULL by the UNIQUE constraint. A table can have multiple UNIQUE keys.
...
A primary key is a column or combination of columns in a table that uniquely identifies each row. Every value in the primary key must be unique across all rows and cannot be NULL. A table can have only one primary key. The primary key serves as the authoritative row identifier and is the main target for foreign key references from other tables.
Example: In an employees table, employee_id INT PRIMARY KEY uniquely identifies each employee.
A foreign key is a column in one table whose values reference the primary key of another table. It establishes a relationship between the two tables and enforces referential integrity by ensuring every foreign key value either matches an existing primary key value in the referenced table or is NULL.
...
Uniqueness: primary key values must be unique per table. Foreign key values can repeat — many rows in the child table can reference the same parent row (for example, many orders can belong to the same customer).
NULL values: the primary key never allows NULL. The foreign key can be NULL, meaning the row is not linked to any parent.
Count: one primary key per table. A table can have multiple foreign keys, each pointing to a different parent table.
Location: the primary key resides in the parent table. The foreign key resides in the child table and points to the parent.

What is a foreign key? How to implement the same in MySQL?
A foreign key is a column (or set of columns) in one table whose values reference the primary key of another table. It creates a logical link between the two tables and enforces referential integrity by ensuring every foreign key value either matches an existing primary key value in the referenced table or is NULL.
...
A foreign key constraint is a rule that links a column (or set of columns) in a child table to the primary key of a parent table. It ensures referential integrity by guaranteeing that every value in the foreign key column either matches an existing primary key value in the parent table or is NULL.
...
No. A table can have only one primary key. This is a fundamental rule of relational database design. However, a primary key can be a composite key — made up of two or more columns together — but it still counts as a single primary key.
Alternate Key: An alternate key is a candidate key that was not selected as the primary key.
To understand this, a candidate key is any column or combination of columns that can uniquely identify every row in the table and cannot be NULL. A table can have multiple candidate keys.
...
A UNIQUE key is a constraint that ensures every value in the constrained column is distinct — no two rows can have the same value. A column with a UNIQUE constraint can still hold NULL values (typically one NULL is allowed per unique column).
...
TRUNCATE is a DDL (Data Definition Language) command. It removes all rows from a table by deallocating the data pages in one operation. TRUNCATE cannot include a WHERE clause — it always removes every row. TRUNCATE is auto-committed and generally cannot be rolled back. It does not fire row-level triggers. TRUNCATE is significantly faster than DELETE for large tables because it does not produce individual row-deletion log entries. TRUNCATE also resets the AUTO_INCREMENT counter to its initial value.
Key differences: DELETE can target specific rows with WHERE; TRUNCATE always removes all rows. DELETE can be rolled back; TRUNCATE is permanent. DELETE is slower on large tables; TRUNCATE is much faster. DELETE fires triggers; TRUNCATE does not.

What is the difference between isnull operator() and ifnull() function?
ISNULL(expression) takes exactly one argument and checks whether that expression is NULL. It returns 1 if the value is NULL or 0 if it is not NULL. ISNULL is a test — it does not replace the NULL with anything, it simply tells you whether the value is NULL.
Example: SELECT ISNULL(NULL); returns 1. SELECT ISNULL(5); returns 0.
...
The core difference is purpose and behaviour. ISNULL answers the question of whether a value is NULL by returning 0 or 1. IFNULL answers the question of what to display when a value is NULL by returning a fallback. ISNULL is used in conditional logic; IFNULL is used to substitute meaningful defaults for NULL values in output.
The key difference is in purpose and number of arguments. ISNULL takes one argument and answers the question is this NULL with a 0 or 1. IFNULL takes two arguments and answers what value should be shown when this is NULL by substituting a meaningful default. ISNULL is used for checking; IFNULL is used for handling NULL values in output.

Mention the differences between UNION and UNION ALL.
UNION combines the results of two or more SELECT statements into one result set and automatically removes any duplicate rows. The output contains only unique rows.
UNION ALL combines the results of two or more SELECT statements and keeps every row including duplicates. No deduplication step is performed.
...
Both RANK() and DENSE_RANK() are window functions that assign a numerical rank to each row based on a specified ordering. They differ in how they handle ties.
...
A nested subquery (non-correlated subquery) is an inner query that is completely independent of the outer query. It does not reference any column from the outer query. It can be executed on its own and produces a fixed result. The nested subquery runs once, its result is used by the outer query for all rows.
Example of a nested subquery: SELECT employee_id, salary FROM employees WHERE salary > (SELECT AVG(salary) FROM employees); The inner query SELECT AVG(salary) FROM employees runs once and returns a single value. The outer query uses that fixed value to filter all employees.
A correlated subquery references at least one column from the outer query. Because it depends on the outer query's current row, it cannot run independently. It is re-executed once for every row processed by the outer query, each time using the current row's values.
Example of a correlated subquery: SELECT employee_id, salary, department_id FROM employees e WHERE salary > (SELECT AVG(salary) FROM employees WHERE department_id = e.department_id); For each employee in the outer query, the inner query runs with that employee's department_id, calculating the average salary for that specific department.
Key differences: A nested subquery runs once; a correlated subquery runs once per outer row. Nested subqueries are independent and can be tested alone; correlated subqueries depend on the outer query. Nested subqueries are generally faster; correlated subqueries can be slow on large tables because of repeated execution. Nested subqueries are simpler to read; correlated subqueries are more powerful for per-row comparisons.

What is a correlated subquery?
A correlated subquery is a subquery that references one or more columns from the outer (enclosing) query. Because it uses a value from the outer query's current row, the inner query cannot be executed independently — it must be re-evaluated once for every row the outer query processes.
In contrast, a regular (non-correlated) subquery is fully independent of the outer query and runs only once.
...
A correlated subquery in the HAVING clause is one where the inner query references a value from the outer query's current group. HAVING evaluates the subquery after GROUP BY has created the groups, so the correlated reference is to aggregate or group-level values.
...
For each group (department), HAVING evaluates the subquery using that group's aggregated context, keeping only departments whose employee count exceeds the average count across all departments. The subquery is re-evaluated for each group, which is the defining characteristic of a correlated usage.
...
Result shape: A JOIN expands the row width by adding columns from the joined table. A subquery in WHERE only filters rows — it does not add columns from the subquery.
...
WHERE filters individual rows from the table based on a condition. It runs before any grouping takes place. WHERE works on every raw row of the table one at a time. Rows that do not satisfy the WHERE condition are excluded from all further processing. Aggregate functions cannot be used in WHERE.
Example: SELECT * FROM employees WHERE salary > 50000; returns only individual employee rows where the salary exceeds 50000.
GROUP BY organizes the remaining rows (after WHERE) into groups based on matching values in one or more columns. Each group collapses into a single summary row when combined with aggregate functions like COUNT(), SUM(), AVG(). GROUP BY is executed after WHERE.
...
WHERE filters individual rows before grouping. It is evaluated before GROUP BY. WHERE cannot contain aggregate functions because aggregation has not yet happened. WHERE removes specific rows from the table before any groups are formed.
...
No. Aggregate functions such as COUNT(), SUM(), AVG(), MIN(), and MAX() cannot be used directly inside the WHERE clause.
...
The query is incorrect because COUNT(*), an aggregate function, is used inside the WHERE clause. SQL does not allow aggregate functions in the WHERE clause.
...
The WITH clause defines a Common Table Expression (CTE), which is a temporary named result set that exists only for the duration of the query. The CTE is defined at the beginning of the query and can be referenced one or more times within the main SELECT, UPDATE, INSERT, or DELETE that follows.
Syntax: WITH cte_name AS ( SELECT columns FROM table WHERE condition ) SELECT * FROM cte_name WHERE further_condition;
Benefits of the WITH clause: Improves readability by breaking a complex query into named, understandable steps. Avoids repeating the same subquery multiple times in a single statement. Allows multiple CTEs to be chained, with each CTE able to reference previously defined CTEs. Supports recursive queries for hierarchical data (WITH RECURSIVE).
...
A cross join produces the Cartesian product of two tables. It combines every row from the first table with every row from the second table. If the first table has M rows and the second has N rows, the result contains M multiplied by N rows.
A cross join does not require any join condition — there is no ON clause. Every possible combination of rows from both tables is returned.
...
The HAVING clause filters groups of rows after the GROUP BY clause has created them. It plays the same role for grouped results that WHERE plays for individual rows. WHERE cannot be used here because aggregate functions are evaluated after grouping, and HAVING is evaluated after GROUP BY.
...
Window functions (also called analytical functions) perform calculations across a set of related rows — the window — while keeping each row individually visible in the output. Unlike GROUP BY, they do not collapse rows into a single summary row.
Ranking rows within groups: RANK(), DENSE_RANK(), and ROW_NUMBER() assign a rank to each row within a partition. For example, ranking employees by salary within each department without collapsing the rows.
...
Accessing previous or next rows: LAG() retrieves the value from a prior row and LEAD() retrieves the value from a following row. This is used to compare this month's sales to last month without a self-join.
...
The key advantage is that all of these calculations return a result for every row alongside the original row data, something that GROUP BY aggregation cannot do.

Explain about EXISTS operator with example
The EXISTS operator tests whether a subquery returns at least one row. It returns TRUE if the subquery produces one or more rows and FALSE if it produces no rows. EXISTS does not return the actual column values from the subquery — it is purely a membership check.
...
NOT EXISTS is the negation. It returns TRUE when the subquery returns no rows, allowing you to find records that have no match in another table. Example: Find customers who have never placed an order. SELECT customer_id FROM customers WHERE NOT EXISTS (SELECT 1 FROM orders WHERE orders.customer_id = customers.customer_id);
...
Locks are database mechanisms that control concurrent access to data. When multiple transactions attempt to read or modify the same data simultaneously, locks prevent conflicts and maintain data consistency.
Locking levels: Row-level lock locks a single row, allowing other transactions to access all other rows in the same table simultaneously. This is the most granular lock and causes the least contention. Page-level lock locks a block of rows stored together on a data page. Table-level lock locks the entire table, blocking other transactions from accessing any row in it. Simple to manage but significantly reduces concurrency.
Types of locks:
...
UNIQUE ensures every value in the column is distinct across all rows. No two rows can share the same value. A UNIQUE column can still hold NULL values. Example: CREATE TABLE users (email VARCHAR(100) UNIQUE);
...
FOREIGN KEY links a column to the primary key of another table, enforcing referential integrity. Values in the foreign key column must exist in the referenced table or be NULL. Example: CREATE TABLE orders ( order_id INT PRIMARY KEY, customer_id INT, FOREIGN KEY (customer_id) REFERENCES customers(customer_id) );
...
What is meant by transaction? What are ACID properties? What is the ACID property in a database?
A transaction is a single logical unit of work that groups one or more SQL operations so that they all succeed or all fail together. A transaction is treated as indivisible — if any operation in it fails, the entire transaction is rolled back to the state before it began.
...
Atomicity means the transaction is treated as a single unit. Either all statements in the transaction execute and are committed, or none of them take effect. Partial completion is not allowed.
...
Isolation means concurrent transactions do not interfere with each other. The intermediate state of one transaction is invisible to other transactions. Each transaction behaves as if it runs in isolation, even when hundreds of transactions execute simultaneously.
Durability means that once a transaction is committed, its changes are permanent. The committed data is written to persistent storage and will survive system failures such as crashes, power cuts, or server restarts.
...
Atomicity: A transaction is treated as a single indivisible unit of work. Either every operation in the transaction is executed and committed, or none of them take effect. If a transaction fails midway, all completed operations within it are automatically rolled back. This prevents partial updates that would leave the database in a corrupted state. For example, if a bank transfer debits one account but fails before crediting another, Atomicity ensures the debit is also reversed.
...
Durability: Once a transaction is committed, its changes are permanent. The data is written to persistent storage and will survive system failures, crashes, or power losses. A committed transaction cannot be undone by any subsequent failure.
Together, ACID properties guarantee that the database behaves reliably and predictably even when errors occur or many transactions run simultaneously.

What are the functions of commit and rollback statements?
COMMIT ends the current transaction and permanently saves all the changes made during that transaction to the database. Once a transaction is committed, the changes are written to disk and become visible to other transactions. A committed transaction cannot be undone using ROLLBACK.
...
The CONCAT() function is used to join two or more strings together into a single string in MySQL.
...
If any argument passed to CONCAT() is NULL, the function returns NULL for the entire result. To handle NULLs gracefully, CONCAT_WS() (Concatenate With Separator) is used instead. It takes a separator as the first argument and ignores NULL values among the remaining arguments.
...
You can rename multiple tables in a single atomic statement: RENAME TABLE employees TO staff, departments TO divisions;
An alternative is the ALTER TABLE statement with RENAME TO: ALTER TABLE employees RENAME TO staff;
Both approaches produce the same result. RENAME TABLE is preferred when renaming multiple tables at once because the entire operation is atomic — either all tables are renamed successfully or none are renamed at all, preventing situations where only some renames complete.

What is the view? How can you create and drop view in MySQL?
A view is a virtual table in MySQL. It does not physically store data. Instead, it stores a SQL SELECT query. When you query a view, MySQL executes the underlying SELECT statement and presents the results as if they were coming from a real table. A view behaves like a table for all read operations.
...
Dropping a view removes only the view definition from the database. The underlying base tables and their data are not affected in any way.

What are views in MySQL and why are they used?
A view in MySQL is a virtual table defined by a stored SQL SELECT query. It does not physically store any data. When you query a view, MySQL executes the underlying SELECT statement at that moment and returns the results as if they came from a regular table.
Why views are used:
...
Security and access control: a view can expose only selected columns of a table, hiding sensitive data like passwords, salaries, or personal identifiers. Database administrators can grant users access to the view without granting access to the underlying tables.
...
Data abstraction: if the underlying table structure changes — columns renamed, tables split — only the view definition needs updating. Applications using the view continue to work without modification.
...
DROP VIEW view_name removes the view definition named view_name from the database. The view itself — which is just a stored SELECT query — is deleted from the database catalog. The underlying base tables and all data in them are completely unaffected.
Regarding updating views — it depends on the type of view:
Simple views can be updated. A simple view is created from a single table using a basic SELECT without GROUP BY, DISTINCT, aggregate functions, UNION, or subqueries. You can perform INSERT, UPDATE, and DELETE on a simple view and the changes are applied directly to the underlying base table.
...
Complex views cannot be updated. A view is not updatable if it uses: GROUP BY or HAVING. Aggregate functions such as SUM, COUNT, AVG. The DISTINCT keyword. UNION or UNION ALL. Subqueries in the SELECT list. Joins in most circumstances.
The reason complex views cannot be updated is that MySQL cannot always determine which base table rows to modify when the view is built from multiple tables or derived computations.
The WITH CHECK OPTION clause can be added to a simple view to ensure that any INSERT or UPDATE done through the view must still satisfy the view's WHERE condition. If an updated row would no longer appear in the view, the operation is rejected.

What is a view? Mention differences between Simple and Complex view.
A view is a virtual table whose content is defined by a stored SQL SELECT query. It does not physically store any data. When you query a view, MySQL executes the underlying SELECT and returns the results as if they came from a real table.
...
A Complex View is created using multiple tables, or involves GROUP BY, aggregate functions, DISTINCT, UNION, or subqueries. Complex views are generally not updatable because MySQL cannot always determine how to map the change back to the correct rows in the base tables.
...
Key differences: simple views use one table; complex views can use many. Simple views support DML; complex views generally do not. Simple views have no aggregation; complex views can. Simple views are easy to maintain; complex views encapsulate sophisticated business logic.

What is a view, and why use it? Can we create a view based on another view?
A view is a virtual table defined by a stored SQL SELECT query. It does not physically store data. When you query a view, MySQL executes its underlying SELECT at that moment and returns the result as if it came from a real table.
Why use views:
...
Security: views can expose only selected columns while keeping sensitive columns hidden. Access can be granted to a view without granting access to the base tables.
...
Abstraction: if underlying table structures change, only the view definition needs updating, not every query that uses the view.
Consistency: a view guarantees that every user querying it gets results from the same verified logic.
Can a view be created based on another view? Yes. A view can reference another view instead of a base table. This is called a nested view or chained view.
...
The second view references the first. When very_high_earners is queried, MySQL resolves the full chain of views back to the base table and executes the combined logic.
...
Functional Dependency is the foundation of normalization. A column B is functionally dependent on column A if knowing A determines the value of B. For example, knowing an employee_id determines the employee_name.
...
Second Normal Form (2NF) requires the table to already satisfy 1NF and additionally every non-key column must depend on the complete primary key, not just part of it. Partial dependencies must be removed. This is relevant only when the primary key is composite (made up of more than one column).
Third Normal Form (3NF) requires the table to satisfy 2NF and additionally there must be no transitive dependencies. A transitive dependency is when a non-key column depends on another non-key column instead of directly on the primary key. For example, if a table has employee_id, department_id, and department_name, and department_name depends on department_id rather than on employee_id, that is transitive and should be moved to a separate departments table.
...
Improves data integrity. Clear dependencies between tables and enforced foreign key relationships ensure that data remains accurate and consistent.
...
Referential integrity is a database property that ensures the relationships between tables remain valid and consistent at all times. It guarantees that a foreign key value in a child table always corresponds to an existing primary key value in the parent table, or is NULL.
Referential integrity prevents two categories of problems:
Orphan records occur when a row in the child table references a primary key value that no longer exists in the parent table. For example, an order record containing a customer_id that was deleted from the customers table would be an orphan.
...
In MySQL, referential integrity is enforced by defining FOREIGN KEY constraints. When a foreign key is declared, the database automatically validates every INSERT and UPDATE to the child table to confirm that the referenced value exists in the parent.
...
Yes, GROUP BY can be applied to the First Name column even when it contains duplicate values and NULL values, and it handles both correctly.
...
NULL values: GROUP BY treats all NULL values as belonging to the same group. All rows where First Name is NULL are grouped together as one NULL group. This is a deliberate behavior in MySQL — even though NULL does not equal NULL in comparisons (NULL = NULL is UNKNOWN), GROUP BY treats NULLs as equal for grouping purposes.
...
You control child table behavior using the ON DELETE and ON UPDATE options in the FOREIGN KEY constraint. These options specify what action the database should take on the child rows automatically when the referenced parent row is deleted or updated.
Available actions:
CASCADE: The change is automatically propagated to the child table. Deleting a parent row deletes all matching child rows. Updating the parent key updates all matching child foreign key values.
...
Multi-row operators are used when a subquery returns more than one row. Standard single-value comparison operators like equals or greater-than cannot handle multi-row results. Multi-row operators are designed to evaluate a value against a set of values.
IN checks whether a value matches any value in the set returned by the subquery. Example: SELECT employee_id, name FROM employees WHERE department_id IN (SELECT department_id FROM departments WHERE location = 'Mumbai'); Returns all employees who belong to any department located in Mumbai.
NOT IN returns rows where the value does not match any value in the subquery result. Example: SELECT customer_id FROM customers WHERE customer_id NOT IN (SELECT customer_id FROM orders); Returns customers who have never placed an order.
ANY (equivalently SOME) returns TRUE if the comparison is true for at least one value in the subquery result. Example: SELECT salary FROM employees WHERE salary > ANY (SELECT salary FROM employees WHERE department_id = 5); Returns employees whose salary is greater than at least one salary in department 5.
ALL returns TRUE only if the comparison is true for every value in the subquery result. Example: SELECT salary FROM employees WHERE salary > ALL (SELECT salary FROM employees WHERE department_id = 5); Returns employees whose salary is greater than every salary in department 5 — effectively employees earning more than the department 5 maximum.
...
When you need columns from multiple tables in the result set. A JOIN naturally combines columns from all joined tables in one output row. A subquery in the WHERE clause cannot return columns from the subquery table alongside the outer query columns.
...
When the relationship is a simple foreign key match. Linking a child table to its parent through a JOIN on the foreign key is the natural and intended way to retrieve related data.
...
Subqueries remain appropriate when: the logic is a filter based on a calculated value (salary greater than the average), when using NOT IN or NOT EXISTS to find non-matching records, when the result needed is a scalar value for comparison, or when the inner query is logically independent of the outer query and the intent is clearer as a subquery.
...
LPAD(string, length, pad) and RPAD() pad a string to a specified length by prepending or appending a pad string. Example: SELECT LPAD('5', 4, '0'); returns 0005.
...
Table-level CHECK (can reference multiple columns): CREATE TABLE products ( product_id INT PRIMARY KEY, price DECIMAL(10,2), discounted_price DECIMAL(10,2), CONSTRAINT chk_discount CHECK (discounted_price < price) ); This ensures the discounted price is always lower than the original price.
...
Note: MySQL fully enforces CHECK constraints from version 8.0.16 onwards. In earlier versions they were accepted syntactically but not actually enforced.
...
Row-level detail is preserved. Unlike GROUP BY which collapses multiple rows into one summary row, analytical functions return a value for every row while also computing group-level statistics. You can see both individual data and summary values in the same result without a second query.
Complex self-joins are eliminated. Before window functions, getting a running total or accessing a value from a previous row required joining a table to itself, which was verbose and hard to debug. Window functions achieve this in a single, readable expression.
...
Powerful ranking within groups. RANK(), DENSE_RANK(), and ROW_NUMBER() assign ranks within a partition without any complex self-join. Identifying the top three products per category is a single query.
...
A LEFT JOIN (LEFT OUTER JOIN) returns all rows from the left (first) table and the matching rows from the right table. Where no match exists in the right table, the columns from the right table appear as NULL in the result. The left table rows are never excluded.
...
The key practical difference: INNER JOIN keeps only matched rows from both sides. LEFT JOIN always keeps all rows from the left table and fills in NULL where the right side has no match. LEFT JOIN is especially useful for finding records in one table that have no corresponding records in another, by filtering for WHERE right_table.id IS NULL after the join.

What are SQL aggregate functions? List at least three aggregate functions and describe their purposes.
Aggregate functions are built-in SQL functions that perform a calculation on a set of rows and return a single summary value. They operate on groups of rows, ignoring NULL values in most cases, and are commonly used with GROUP BY.
COUNT() counts the number of rows or non-NULL values. COUNT(*) counts all rows including those with NULL values. COUNT(column) counts only rows where the specified column is not NULL. Example: SELECT COUNT(*) FROM employees; returns the total number of employees. Example: SELECT department_id, COUNT(*) FROM employees GROUP BY department_id; counts employees per department.
...
MIN() returns the smallest non-NULL value in a column. Works on numbers, text (alphabetically), and dates. Example: SELECT MIN(hire_date) FROM employees; returns the earliest hiring date.
MAX() returns the largest non-NULL value in a column. Example: SELECT MAX(salary) FROM employees; returns the highest salary.
All aggregate functions except COUNT(*) ignore NULL values in their computation.

How does the GROUP BY clause work in SQL? In which situations would you use it?
The GROUP BY clause organizes rows that share the same value in one or more specified columns into groups. Once rows are grouped, aggregate functions like COUNT(), SUM(), AVG(), MIN(), MAX() are applied to each group separately, and one summary row is produced per group.
How it works: SQL first filters rows with WHERE, then GROUP BY collects rows sharing the same value in the grouping columns, then aggregate functions compute results within each group, then HAVING optionally filters those groups, and finally the result is returned.
...
The ORDER BY clause sorts the rows in the result set of a SELECT query by one or more specified columns. Without ORDER BY, SQL does not guarantee any particular order for the rows returned — the database can return them in any order.
Ascending order (ASC) sorts rows from the smallest value to the largest. For numbers, this is lowest to highest. For text, this is alphabetical from A to Z. For dates, this is earliest date first. ASC is the default and does not need to be written explicitly. Example: SELECT * FROM employees ORDER BY salary; or SELECT * FROM employees ORDER BY salary ASC; — both list employees from lowest to highest salary.
...
NULL handling: in MySQL, NULL values are treated as lower than any non-NULL value in ascending order (they appear first) and as higher in descending order (they appear last).
...
A subquery is a SELECT statement nested inside another SQL statement, called the outer query. The subquery executes first and its result is passed to the outer query. Subqueries can appear in the WHERE clause, FROM clause, SELECT list, or HAVING clause.
Types of subqueries: Single-row subquery returns one row and one column, used with = or comparison operators. Multi-row subquery returns multiple rows, used with IN, ANY, ALL, or EXISTS. Correlated subquery references a column from the outer query and re-executes for each outer row. Scalar subquery returns a single value and can appear in the SELECT list.
Example situation 1 — Filtering based on an aggregate value: Find all employees earning more than the company average salary. SELECT employee_id, first_name, salary FROM employees WHERE salary > (SELECT AVG(salary) FROM employees); The subquery computes the average once; the outer query uses it to filter employees. This cannot easily be done without a subquery.
...
The DISTINCT keyword eliminates duplicate rows from the result set of a SELECT query. When included, only unique combinations of the selected columns are returned — if two or more rows would produce identical values in all selected columns, only one of them appears in the output.
Syntax: SELECT DISTINCT column1, column2 FROM table;
Effect on results: Without DISTINCT: SELECT department_id FROM employees; returns one row per employee. If 50 employees are spread across 5 departments, 50 rows are returned with many repeated department IDs. With DISTINCT: SELECT DISTINCT department_id FROM employees; returns 5 rows — one for each unique department ID.
